...
const guides: Guide[] = [
  ...
  },
  {
    id: 'g6', icon: '📚', title: 'Tracker Materi & Estimasi Pertemuan', steps: [
      ...
      { t: 'Butuh waktu lebih banyak', d: 'Ubah estimasi pertemuan dari Hari Ini atau <strong>Progres</strong>, lalu naikkan jumlahnya. Riwayat yang sudah tersimpan tidak berubah.' },
      ...
      { t: 'Perbaiki salah input', d: 'Buka <strong>Progres → Riwayat</strong> untuk menelusuri dan memperbaiki catatan sesi.' },
    ], tip: 'Estimasi adalah rencana, bukan aturan wajib. Progres mengikuti materi yang benar-benar diajarkan di kelas.'
  },
  {
    id: 'g2', icon: '📈', title: 'Cara Membaca Progres', steps: [
  ...
  },
  {
  ...
  },
    {
      id: 'g5', icon: '✨', title: 'Fitur Unggulan EduTrack', steps: [
        ...
        { t: 'Riwayat Sesi Bulanan', d: 'Buka <strong>Progres → Riwayat</strong> untuk menelusuri kegiatan mengajar di waktu lampau.' },
...
export default function InfoView({ onBackToSetup, backLabel = 'Kembali ke Kelola' }: { onBackToSetup: () => void; backLabel?: string }) {
  ...
  const featureGroups: { title: string; icon: ElementType; items: { icon: ElementType; t: string; d: string }[] }[] = [
    {
      ...
      items: [
        { icon: Lightbulb, t: 'Progres Pengajaran', d: 'Pantau materi, pertemuan, catatan berikutnya, dan kondisi sesi setiap kelas.' },
  ...
  const quickHelp = [
    ...
    { icon: TrendingUp, t: 'Pantau Progres', d: 'Cek status mapel yang aman, mepet, atau kurang sesi.' },
      ...
      <div className="app-card p-5 mb-4 relative overflow-hidden">
        ...
        <div className="relative flex items-start gap-3">
          ...
          <div>
            ...
            <div className="text-[13px] text-text2 leading-relaxed mt-2">Bagian dari Kelola. Panduan operasional untuk mengatur data, memantau Progres, dan menjaga backup tetap aman.</div>
          </div>
        </div>
          ...
          {quickHelp.map(item => {
            ...
            return (
              <div key={item.t} className="bg-surface2/60 border border-border/60 rounded-2xl p-3 text-center">
                ...
                <div className="text-xs font-black text-foreground leading-tight">{item.t}</div>
              </div>
            );
          })}
        </div>
                    ...
                    <div className="p-4 border-t border-border/40">
                      {g.steps.map((s, i) => (
                        <div key={i} className="flex items-start gap-3 mb-3 last:mb-0">
                          <div className="w-6 h-6 rounded-full bg-surface2 text-text3 text-xs font-bold grid place-items-center flex-shrink-0 mt-0.5">{i + 1}</div>
