        ...
        {tabs.map(item => {
          ...
          return (
            <button
              ...
              className={`flex min-h-[38px] flex-1 items-center justify-center gap-1.5 rounded-xl text-[11px] font-black uppercase tracking-wide transition ${
    ...
    <div className="space-y-4">
      <section className="app-card p-3 sm:p-4">
        ...
        <select
          ...
          className="mt-1.5 w-full rounded-xl border border-border bg-surface px-3 py-3 text-sm font-bold outline-none focus:border-primary"
        >
          {classes.map(c => (
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 font-display text-xl font-bold">{selectedClass?.name ?? 'Pilih kelas'}</h2>
          </div>
...
function SubjectCard({
  ...
  return (
    <>
      <article
        className={`overflow-hidden rounded-2xl border bg-surface/90 shadow-md transition-all ${
          tone === 'red' ? 'border-red/30' : tone === 'amber' ? 'border-amber/30' : 'border-border/60'
        }`}
      >
        <div className="p-4 sm:p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="truncate text-base font-black tracking-tight">{subjectName}</h3>
          ...
          {/* BAB SAAT INI (Hero Card Section) */}
          <div className="mt-4 rounded-xl border border-border/70 bg-surface2/50 p-3.5 shadow-inner">
            ...
            {position.isComplete ? (
              ...
              <>
                ...
                <div className="mt-1 flex items-center gap-2 flex-wrap text-[11px]">
                  <span className="font-extrabold text-primary bg-primary/10 border border-primary/20 rounded-md px-2 py-0.5">
          ...
          {/* PERTEMUAN BERIKUTNYA (Catatan & Topik Mengajar) */}
          {activeMaterial && !position.isComplete && (
            <div className="mt-3 rounded-xl border border-primary/20 bg-primary/5 p-3.5 transition-all">
              <div className="flex items-center justify-between gap-3">
                ...
                {!editingNote && (
                  <button
                    ...
                    className="flex items-center gap-1 rounded-lg border border-primary/30 bg-surface px-2.5 py-1 text-[10px] font-bold text-primary shadow-xs hover:bg-primary/10 transition-colors"
                  >
                    <Pencil className="h-3 w-3" /> Edit catatan
                  </button>
                )}
              </div>
                ...
                <div className="mt-3 space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="min-h-[70px] w-full resize-none rounded-xl border border-primary/30 bg-surface p-2.5 text-xs outline-none focus:border-primary"
                    />
                  </div>
                  <div className="flex justify-end gap-2 pt-1">
                    <button
                      ...
                      className="px-3 py-1.5 text-[11px] font-bold text-text3 hover:text-foreground"
                    >
                      Batal
                    </button>
                    <button
                      ...
                      disabled={isSavingNote}
                      className="flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-1.5 text-[11px] font-black text-primary-foreground shadow-sm disabled:opacity-50"
                    >
                ...
                <div className="mt-2.5 space-y-2 text-xs">
                  {parsedNote.mainNote && (
                    <div className="rounded-lg bg-surface/60 p-2 border border-border/40">
                      ...
                      <p className="mt-0.5 font-bold text-foreground">{parsedNote.mainNote}</p>
                    </div>
                  )}
                  {parsedNote.reminder && (
                    <div className="rounded-lg bg-surface/60 p-2 border border-border/40">
                      <span className="block text-[9px] font-black uppercase tracking-wider text-amber">Catatan Mengajar</span>
                      <p className="mt-0.5 font-medium text-text2 whitespace-pre-wrap">{parsedNote.reminder}</p>
                    </div>
                  )}
          ...
          <button
            ...
            className="mt-3 flex w-full items-center justify-between rounded-xl border border-border/60 bg-surface px-3 py-2.5 text-left text-[11px] font-black text-text2 hover:bg-surface2/60 transition-colors"
          >
        ...
        {/* Collapsible Content: Rencana Bab */}
        {expanded && (
          <div className="border-t border-border/50 bg-surface2/30 p-4">
            <div className="space-y-2.5">
              {(() => {
                ...
                return materials.map(m => {
                  ...
                  consumed += count;
                  const editing = editingSessionsId === m.id;
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2">
                            {editing ? (
                              <div className="flex items-center gap-1.5">
                                <input
                                  ...
                                  className="h-8 w-16 rounded-lg border border-primary/40 bg-surface px-2 text-center text-xs font-bold outline-none focus:border-primary"
                                  autoFocus
                                />
                                <button
                                  onClick={() => saveSessions(m)}
                                  disabled={isSavingSession}
                                  className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-primary-foreground hover:brightness-105"
                                  title="Simpan"
                                >
                                  {isSavingSession ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                                </button>
                                <button
                                  onClick={() => setEditingSessionsId(null)}
                                  className="grid h-8 w-8 place-items-center rounded-lg border border-border text-text3 hover:bg-surface2"
                                  title="Batal"
                                >
                              ...
                              <button
                                onClick={() => {
                                  setEditingSessionsId(m.id);
                                  setSessionDraft(String(count));
                                }}
                                className="rounded-lg border border-border bg-surface2 px-2.5 py-1 text-[10px] font-bold text-text2 hover:border-primary/40 transition-colors"
                              >
                                {count} pertemuan ✏️
                              </button>
                            )}
                            ...
                            {current && (
                              <button
                                ...
                                className="rounded-lg border border-amber/30 bg-amber/10 px-2.5 py-1 text-[10px] font-black text-amber hover:bg-amber/20 transition-colors flex items-center gap-1"
                              >
            ...
            <div className="mt-4 border-t border-border/50 pt-3 space-y-3">
              ...
              {sessionsDone > 0 && (
                <div className="flex items-center justify-between border-t border-border/50 pt-3">
                  ...
                  <button
                    onClick={undo}
                    className="rounded-lg border border-amber/25 bg-amber/10 px-2.5 py-1.5 text-[10px] font-black text-amber hover:bg-amber/20 transition-colors flex items-center gap-1"
                  >
            ...
            <div className="app-bottom-sheet" onClick={e => e.stopPropagation()}>
              ...
              <div className="mt-5 flex gap-3">
                <button
                  ...
                  className="flex-1 rounded-xl border border-border2 bg-surface py-3 text-sm font-bold text-text2"
                >
                  Batal
                </button>
                <button
                  ...
                  disabled={isProcessingFinish}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-black text-primary-foreground shadow-md disabled:opacity-50"
                >
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between rounded-2xl border border-border bg-surface p-3">
        ...
        <input type="month" value={month} onChange={e => setMonth(e.target.value)} className="form-input-style min-h-0 w-auto px-3 py-1.5 text-xs" />
      </div>
      {!dates.length ? (
