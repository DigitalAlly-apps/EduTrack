    ...
    {section === 'calendar' ? <><CalendarTab revision={revision} classId={classFilter} onRepair={date => { setRepairDate(date); setSection('history'); }} /><WeeklyReviewCard key={revision} /></> : <div className="grid items-start gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
      <section className={selected ? 'hidden lg:block' : ''} aria-label="Daftar kelas dan mata pelajaran">
        ...
        <div className="divide-y divide-border rounded-2xl border border-border overflow-hidden">
          {visible.map(({ cls, subject, position, status }) => <button key={`${cls.id}:${subject.id}`} className={`w-full p-4 text-left ${selected?.cls.id === cls.id && selected?.subject.id === subject.id ? 'bg-primary/10' : 'bg-surface hover:bg-surface2'}`} onClick={() => navigateTo({ view: 'progress', classId: cls.id, subjectId: subject.id, section: 'summary' })}>
      ...
      <section className="min-w-0 space-y-4" aria-label="Detail progres">
